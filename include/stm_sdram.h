/** @file stm_sdram.h @brief 平台无关 SDR SDRAM 初始化、访问与显式自检。 */
#ifndef STM_SDRAM_H
#define STM_SDRAM_H
#include <stddef.h>
...
{
#endif
#define STM_SDRAM_VERSION "4.0.0"
    typedef struct sdram_context *sdram_handle_t;
    typedef struct
    {
        sdram_controller_t controller;
        const sdram_device_t *device; /* 必填，创建时复制参数；不自动猜测 SDRAM 型号。 */
    } sdram_config_t;
    typedef struct
    {
        uint8_t row_bits, column_bits, internal_banks, bus_width_bits, cas_latency;
        uintptr_t base;
        uint32_t size_bytes, clock_hz;
        uint32_t refresh_count; /* 适配器返回的编程值，仅诊断用途。 */
        stm_err_t last_error;
        uint8_t ready;
    } sdram_info_t;
    /* MPU/Cache/堆注册由 BSP 管理；只支持调用方确保可访问的非缓存 Normal 映射。 */
    // 自检首次数据不匹配的位置与内容
    typedef struct
    {
     ...
     * @param[in] config 创建配置，仅在调用期间读取；控制器上下文由板级持有。
